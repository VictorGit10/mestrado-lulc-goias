...
**A fronteira de 2020 é robusta — não some com a correção, fortalece** (F 21,5 → 34,1); a série corrigida `agric∪mosaico` sozinha quebra exatamente em 2020 (F=39,8, p=2·10⁻⁷). O que a mudança de rótulo faz é **inverter o sinal** da mudança, não criar a quebra:
...
**Status dos outros testes de quebra sob a mudança de rótulo:**

| Teste | Usa | Status sob a mudança de rótulo |
|---|---|---|
...
| **KL/TV (#29c)** | **matriz de transição, 6 classes** | **CONTAMINADO** — as 6 classes **não rastreiam o Mosaico**, então a mudança de rótulo muda a matriz ~2020; o pico 2018-2020 lê o mesmo artefato. **Não é corroboração independente.** |
| STARS (#29b) | Δveg, Δpasto, Δagric | Não sinaliza 2020 (shifts em 2004/06/14, todos anteriores a ela) — inalterado |
| univariado (#26) | Δ por classe | A quebra da **agricultura em 2018** (F=12,3) **é a mudança de rótulo**; veg (1998) e pasto (2001) intactos |
...
> [!WARNING]
> **A caracterização "Conversão seletiva" estava invertida — e o ato foi RENOMEADO em 25/jul/2026.** A leitura de que a agricultura desacelera / "para de crescer" no Ato III (e a nota do marco 2018) é a **assinatura da mudança de rótulo**, não o campo. Sob a régua corrigida a conversão **acelera**. A *fronteira* de 2020 sempre esteve correta; o *rótulo* é que não estava, e passou a **"Conversão acelerada (mascarada)"** no `config_periodos.py` (propagado a CSVs, JSONs da viz, site e docs). ~~**Pendência**: a leitura do Intensity (#31) de "retração da agricultura" em P3 tem a mesma origem e ainda não foi refeita.~~ — ✅ **REFEITA no mesmo dia** (`scripts/intensity_bracket.py`), e devolveu **dois** defeitos: a retração de P3 **inverte** sob o bracket (−84% na régua crua × **+67%** na união) **e** a linha-base `uniform` também perde o fluxo reetiquetado, o que inflava **toda** razão `*_vs_uniform` do Ato III em ~3× — inclusive as de transições imunes. Ver o `[!CAUTION]` da seção [Intensity Analysis](#intensity-analysis-pipeline-31) abaixo, que é onde essa correção mora.
