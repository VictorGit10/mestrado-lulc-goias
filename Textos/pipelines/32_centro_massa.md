...
Para cada variável e cada ano de 1985 a 2024, usamos os **centroides das AMCs** ($x_i, y_i$) em projeção métrica (EPSG:5880 = SIRGAS 2000 / Brazil Polyconic, projeção de compromisso) e o **valor da variável** naquela AMC ($w_i$) como peso.
...
2. **Gradiente latitudinal persistente.** Mesmo todos subindo, a **distância relativa entre eles quase não muda**: o centro da agricultura fica sistematicamente **ao sul** do de pastagem/rebanho em *todos* os anos, num vão de **123 a 135 km**. *(Correção de 19/ago/2026: escrevia-se "1,1° a 1,2° (≈120–130 km)", e a série dá 122,6 a 135,0 — 10 dos 40 anos caíam fora da faixa, inclusive 2024, cujo valor de 135 km o texto já citava adiante.)* Desenha-se um modelo de cinturões ordenados de sul para norte: **agricultura → pastagem/pecuária → vegetação natural**.
...
- **CRS = EPSG:5880** (SIRGAS 2000 / **Brazil Polyconic** — projeção de **compromisso**, nem conforme nem equivalente; a distorção é pequena para a compacta GO). Centroides e distâncias são calculados em metros nessa projeção; para os rótulos de latitude/longitude, os pontos são reprojetados de volta para EPSG:4674. *(Correção jul/2026: rotulava-se "Albers equal-area"; o `pyproj` confirma que 5880 é policônica. Não muda a marcha de ~80 km — o #43 valida a ~1-2 km por pixel —, mas o rótulo estava errado.)*
...
- **A policônica (5880) é projeção de compromisso — não preserva área nem distância exatamente.** Para o deslocamento N–S de dezenas a centenas de km dentro de GO, o erro de escala é pequeno e aceitável para leitura descritiva — mas as distâncias em km não são geodésicas exatas.
