...
## 7b. Pipeline #16 — `construir_painel_unificado.py`

**O que é**: tabela wide única `cd_mun × ano` (9.840 linhas × 66 colunas) que
consolida todas as fontes prontas em formato pronto para regressão pooled,
painel ou inferência espacial (Moran, LISA, `spreg`).
...
**Como rodar**: `python scripts/construir_painel_unificado.py` (depende dos
Pipelines #3, #4, #6, #7).

**Universo**: 246 municípios de Goiás (atual), 40 anos (1985–2024). Linha = município × ano.

**Blocos de variáveis** (prefixo identifica fonte):
...
| `agri_*` (12 cols: soja, milho 1ª/2ª, cana, feijão, algodão × área+ton) | SIDRA PAM 1612 + 839 (Pipeline #3) | 1974+ (safrinha desde 2003) |
...
2. **Deflação**: PIB, VA agro e SICOR convertidos para R$ de **dezembro/2024** via IPCA dez-a-dez (tabela SIDRA 1737). Padrão idêntico ao Pipeline #8.
...
- Não inclui IDH-M e Fogo (slots vazios; pendente Pipelines #13 e #14).
...
- [x] `construir_painel_unificado.py` — Painel Unificado município × ano (Pipeline #16). Consolida LULC + pecuária + lavouras + PIB + população + crédito SICOR + Censo 2017 em tabela wide 9.840 × 66. Slots reservados para IDH-M (#13) e Fogo (#14). Saída em `data/processed/painel_unificado.{parquet,csv}` + diagnóstico de cobertura em `outputs/diagnosticos/painel_unificado_cobertura.csv`. **Destrava** análise espacial estatística (Moran/LISA/regressão).
