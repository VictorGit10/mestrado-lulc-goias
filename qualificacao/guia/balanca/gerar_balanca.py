"""gerar_balanca.py — dados reais para "A balança de Goiás" (explicação visual do #32)
=================================================================================

Material de estudo do caderno de preparação (não entra na entrega). Exporta, dos
MESMOS arquivos e com a MESMA álgebra do Pipeline #32 (centro_massa.py) e do #55
(robustez_bootstrap_bloco.py), tudo o que a página precisa para refazer a conta no
navegador:

  - os 166 polígonos das AMC e seus centroides em EPSG:5880 (km);
  - os pesos w_it das quatro variáveis, 1985–2024;
  - as partições em blocos do #55 (k-means com a mesma semente);
  - os resultados oficiais (CSV do #32/#55/#43) para a página conferir-se contra eles;
  - um ajuste polinomial EPSG:5880 → (lon, lat) para rotular latitude no navegador.

Nada é recalculado para o texto: a página reproduz os números e mostra a diferença.
Ao final, confere que o centro recalculado aqui bate com centro_massa_anual.csv.

COMO RODAR
    python qualificacao/guia/balanca/gerar_balanca.py
Saída: qualificacao/guia/balanca/balanca.html (template.html + dados embutidos).
"""
...
def main() -> None:
    ...
    geo = painel.groupby("code_amc")[["code_amc"]].first()
    ...
    assert "/*__DADOS__*/" in tpl
    out = tpl.replace("/*__DADOS__*/", "const D = " + js + ";")
    (AQUI / "balanca.html").write_text(out, encoding="utf-8")
    print(f"[ok] balanca.html  ({len(out)/1e6:.2f} MB; dados {len(js)/1e6:.2f} MB)")
