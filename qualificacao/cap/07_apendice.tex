...
\toprule
\textbf{Especificação} & \textbf{Regressor} & \textbf{$\hat{\beta}$} & \textbf{EP} & \textbf{$p$} & \textbf{Agrup.} & \textbf{$R^2_w$} & \textbf{$n$} \\
\midrule
B1  fluxo $\sim$ estoque\_def (disponibilidade) & estoque defasado (z) & $+2{,}756$ & $0{,}472$ & $<0{,}001$ & un.+ano & $0{,}434$ & 6.396 \\
B1q fluxo $\sim$ estoque + estoque$^2$ (não-linear) & estoque defasado (z) & $+2{,}686$ & $0{,}476$ & $<0{,}001$ & un.+ano & $0{,}434$ & 6.396 \\
B1q fluxo $\sim$ estoque + estoque$^2$ (não-linear) & estoque defasado$^2$ (z) & $+0{,}045$ & $0{,}451$ & $0{,}920$ & un.+ano & $0{,}434$ & 6.396 \\
B2a hazard $\sim$ estoque\_def & estoque defasado (z) & $-0{,}319$ & $0{,}189$ & $0{,}092$ & un.+ano & $-0{,}009$ & 6.379 \\
B2b hazard $\sim$ depleção\_def & depleção defasada (z) & $-0{,}015$ & $0{,}022$ & $0{,}481$ & un.+ano & $-0{,}000$ & 6.379 \\
B3  fluxo $\sim$ estoque + demanda (sem ano FE) & estoque defasado (z) & $+2{,}763$ & $0{,}391$ & $<0{,}001$ & unidade & $0{,}436$ & 6.396 \\
B3  fluxo $\sim$ estoque + demanda (sem ano FE) & $\Delta$ câmbio real (z) & $+0{,}022$ & $0{,}007$ & $0{,}003$ & unidade & $0{,}436$ & 6.396 \\
B3  fluxo $\sim$ estoque + demanda (sem ano FE) & $\Delta$ preço da soja (z) & $-0{,}030$ & $0{,}006$ & $<0{,}001$ & unidade & $0{,}436$ & 6.396 \\
B3  fluxo $\sim$ estoque + demanda (sem ano FE) & $\Delta$ crédito rural (z) & $+0{,}001$ & $0{,}005$ & $0{,}828$ & unidade & $0{,}436$ & 6.396 \\
\bottomrule
\end{tabular}
\fontefig{elaboração própria.}
\notafig{desfecho: fluxo anual de conversão, exceto nas linhas \texttt{B2a} e \texttt{B2b}, cujo desfecho é a \emph{taxa}. Regressores em \emph{escore-z}. Efeitos fixos de unidade e de ano em todas as linhas \textbf{menos} as do \texttt{B3}, que dispensa o de ano de propósito: os sinais de demanda que ele testa (câmbio, preço, crédito) são nacionais e um efeito fixo de ano os absorveria por inteiro, deixando o coeficiente sem identificação. Esse é também o motivo de os $p$ do \texttt{B3} não serem lidos como o das demais linhas. ``Agrup.'' é a dimensão do agrupamento do erro-padrão, e \textbf{não é a mesma em todas as linhas}: sem efeito fixo de ano, o \texttt{B3} não admite o agrupamento por ano e recai no de unidade --- é a exceção que a Seção~\ref{ap:convencoes} antecipa, e por isso ela é reportada linha a linha e não afirmada uma vez para a tabela inteira. A linha \texttt{B2b} é a especificação que a decisão D29 superou; ver a Tabela~\ref{tab:deplecao}.}
\end{table}
