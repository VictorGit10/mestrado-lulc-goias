% =====================================================================
% Texto de qualificação — Mestrado em Ciências Ambientais
...
\usepackage{booktabs}
\usepackage{xcolor}
\usepackage{placeins}      % \FloatBarrier. Nasceu para o apêndice (o gerador o
                           % emite ao fim de cada seção), onde as tabelas são
                           % muitas e grandes e sem barreira escorriam para
                           % dentro da seção seguinte — a Tabela da A.6 saía
                           % depois de a A.7 já ter começado. Hoje o corpo o usa
                           % em três pontos, cada um comentado no lugar, sempre
                           % onde um flutuante ultrapassava o título seguinte.
                           % Sem a opção [section], que valeria para o documento
                           % inteiro e mexeria na diagramação já conferida.
...
% Entrada do registro de decisões (Apêndice B). Não é quadro flutuante: as 31
% entradas têm comprimento muito desigual (de uma linha a cinco) e ocupam mais
% de uma página, o que um float não atravessa. O recuo pendente alinha o corpo
% da explicação sob o título e deixa o identificador saliente na margem, que é
% o que se procura ao chegar do texto com um número na mão. O verificar.py lê
% estas entradas para conferir que todo DNN citado no corpo tem registro; por
% isso uma decisão por comando.
% O \filbreak mantém a entrada inteira numa página. Num registro que se
% consulta por número, uma entrada de quatro linhas partida ao meio custa
% mais que o vão que sobra ao fim da página anterior, porque quem chega aqui
% com um "D26" na mão quer ler a entrada, e não procurá-la em duas páginas.
\newcommand{\decisao}[3]{%
  \par\filbreak\begingroup\SingleSpacing
  \hangindent=1.5em\hangafter=1\noindent
  \textbf{#1}\hspace{0.6em}\textbf{#2.} #3\par\endgroup
  \vspace{4pt}}
...
\newcommand{\urlrepo}{https://github.com/VictorGit10/mestrado-lulc-goias}
...
% ---------------------------------------------------------------------
% Compilação parcial durante a edição (descomentar e listar o capítulo)
%
% Com \include (e não \input), o LaTeX guarda um .aux por capítulo. O
% \includeonly manda tipografar só os listados, mas continua lendo o .aux
% dos demais -- então a NUMERAÇÃO DE PÁGINA, as referências cruzadas e as
% citações do capítulo em que se está trabalhando saem corretas.
%
% O ganho é modesto: medido aquecido, 5,4 s a passada completa contra
% 3,8 s só o Capítulo 4. Serve para sessão longa dentro de um capítulo
% só; não vale o risco de ficar ligado por engano numa edição rápida.
%
% Comentar de volta antes de gerar a versão de entrega.
%
% \includeonly{cap/04_resultados}
% ---------------------------------------------------------------------
...
\include{cap/00_apresentacao}   % Memorial (não numerado)
\include{cap/01_introducao}
\include{cap/02_referencial}
\include{cap/03_metodologia}
\include{cap/04_resultados}
\include{cap/05_discussao}
\include{cap/06_cronograma}
...
% O apêndice A (07_apendice.tex) é gerado por apendice/gerar_apendice.py a
% partir dos CSVs dos pipelines — não editar à mão. Os apêndices B e C
% (08_decisoes.tex e 09_rotinas.tex) são editoriais e mantidos à mão; a
% distinção está declarada no cabeçalho de cada arquivo.
\begin{apendicesenv}
\partapendices
\include{cap/07_apendice}
\include{cap/08_decisoes}
\include{cap/09_rotinas}
\end{apendicesenv}
