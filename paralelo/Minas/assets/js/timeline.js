 ...
 * Adaptado de Visualizacao para Minas Gerais.
 * Namespace MG40. Sem dados de fogo.
 */
...
(function (root) {
  ...
  // TODO: ajustar eras apos periodizacao de MG. Placeholder usa mesmos limites de GO.
  const ERA_RANGES = [
    {
      era: "heranca",
      ato: "Ato I",
  ...
  async function carregarDados() {
    const [painel, marcos] = await Promise.all([
      fetch("assets/data/painel_mg.json").then(r => r.json()),
      fetch("assets/data/marcos_mg.json").then(r => r.json())
    ]);
...
// -------------------- spark-line --------------------
  // Duas series (sem fogo): soja e rebanho.
  const SPARK_SERIES = [
    {
      chave: "soja",
      campo: "agri_soja_ton",
...
function moverCursorSparkline(ano) {
  ...
  function altDoMapa(camada, ano) {
    switch (camada) {
      case "cobertura":  return `Cobertura LULC em Minas Gerais em ${ano}`;
      case "delta":      return `Variação acumulada da cobertura em Minas Gerais (${ano} vs 1985)`;
      case "transicoes": {
        const p = periodoTransicao(ano);
        return `Transição dominante em Minas Gerais entre ${p.ini} e ${p.fim}`;
      }
      default: return `Mapa de Minas Gerais em ${ano}`;
    }
  }
  ...
  async function init() {
    ...
    root.MG40 = root.MG40 || {};
    root.MG40.narrativa = { aplicarSubrota };
  }
