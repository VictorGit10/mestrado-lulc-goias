 ...
 * Adaptado para MG. Namespace MG40. Sem tema de fogo.
 * Le painel_mg.json e injeta cards tematicos com mini-sparkline.
 */
(function (root) {
  "use strict";
  ...
  const fmtBiL    = v => v == null ? "—" : fmtNum(v / 1e6, 2) + " Bi L";
  const fmtMhaPlt = v => v == null ? "—" : fmtNum(v / 1e6, 2) + " Mha";
  ...
  const TEMAS = [
    {
      ...
      series: [
        ...
        { campo: "lulc_cafe_ha",              nome: "Café (cobertura no mapa)",    format: fmtMha, fonte: "MapBiomas Coleção 10.1 (Pipeline #11)" },
    ...
    {
      id: "cultivos",
      rotulo: "Cultivos · IBGE/SIDRA-PAM",
      series: [
        { campo: "agri_soja_ton",            nome: "Soja (produção)",         format: fmtMt, fonte: "IBGE/SIDRA-PAM (Pipeline #3)" },
        { campo: "perm_cafe_total_ton",      nome: "Café (produção)",         format: fmtKt, fonte: "IBGE/SIDRA-PAM (Pipeline #3)" },
    ...
    },
    {
      ...
      series: [
        { campo: "pib_uf_real_rs",           nome: "PIB de Minas Gerais (UF, IPEA)",        format: fmtBiRs, fonte: "IBGE Contas Regionais · IPEAData" },
    ...
    },
    {
      ...
      series: [
        { campo: "populacao",                nome: "População de Minas Gerais",             format: fmtMhab, fonte: "IBGE — Estimativas e Censos" },
      ],
    },
  ];
  ...
  async function init() {
    ...
    if (!cont) return;
    try {
      const painel = await fetch("assets/data/painel_mg.json").then(r => r.json());
