 ...
 * Adaptado para MG. Namespace MG40. Dados em sankey_ato_I_mg.json etc.
 */
(function (root) {
  "use strict";
  ...
  async function carregarAto(ato) {
    ...
    const resp = await fetch(`assets/data/sankey_ato_${ato}_mg.json`);
    if (!resp.ok) throw new Error(`sankey_ato_${ato}_mg.json: ${resp.status}`);
