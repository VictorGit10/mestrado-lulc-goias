"""gerar_mapas_delta_lulc_mg.py — 40 mapas coropleticos anuais (1985-2024)
de delta % pastagem vs. 1985 por municipio de Minas Gerais.

Adaptado de gerar_mapas_delta_lulc.py para MG.

Saida:
    Minas/img/mapas_delta/delta_{ANO}.webp  (40 arquivos)
"""
...
OUT_DIR = ROOT / "Minas" / "img" / "mapas_delta"


def carregar_malha() -> gpd.GeoDataFrame:
    print("[...] Baixando/carregando malha municipal de Minas Gerais (geobr)...")
    gdf = geobr.read_municipality(code_muni="MG", year=2020)
...
def gerar_mapas(df: pd.DataFrame, gdf_munis: gpd.GeoDataFrame) -> None:
    ...
    for i, ano in enumerate(anos, start=1):
        ...
        ax.set_title(f"Delta % Pastagem — Minas Gerais {ano} ({subtitulo})", fontsize=14, pad=10)
        ...
        ax.set_axis_off()
        adicionar_escala(ax, dx=1, total_km=200)
        adicionar_norte(ax)
...
def main() -> None:
    print("=" * 60)
    print("Gerando 40 mapas coropleticos delta pastagem — MG 1985-2024")
    print("=" * 60)
