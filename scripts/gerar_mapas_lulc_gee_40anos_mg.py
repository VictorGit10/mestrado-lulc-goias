"""gerar_mapas_lulc_gee_40anos_mg.py — 40 mapas raster MapBiomas Coleção 10.1
para Minas Gerais (1985–2024) via Google Earth Engine.

Adaptado de gerar_mapas_lulc_gee_40anos.py para MG.

Saída:
    Minas/img/mapas_gee/cobertura_{ANO}.png   (40 PNGs compostos, DPI 200)
    Minas/img/mapas_gee/_raw/raw_{ANO}.png     (raster cru do GEE, 2048 px)
"""
...
OUT_DIR = ROOT / "Minas" / "img" / "mapas_gee"
...
def init_ee() -> None:
    ...
    if not project:
        sys.exit("Erro: defina GEE_PROJECT ou edite GEE_PROJECT_DEFAULT.")
    try:
...
def carregar_geometria_mg():
    """Lê polígono de Minas Gerais via geobr e converte para ee.Geometry (EPSG:4326)."""
    gdf = geobr.read_state(code_state="MG", year=2020).to_crs(4326)
...
def compor_mapa(raw_bytes: bytes, ano: int, gdf_mg, out_path: Path) -> None:
    img = Image.open(io.BytesIO(raw_bytes)).convert("RGBA")

    fig, ax = plt.subplots(figsize=FIGSIZE)
    ax.imshow(img)
    ax.set_axis_off()
    ax.set_title(f"Cobertura e Uso da Terra — Minas Gerais {ano}", fontsize=14)
...
def main() -> None:
    ...
    gdf_mg_local = _geobr.read_state(code_state="MG", year=2020).to_crs(4326)
    n_anos = ANO_MAX - ANO_MIN + 1
    print(f"Iniciando {n_anos} anos ({ANO_MIN}–{ANO_MAX}) para Minas Gerais.")
    ...
    for i, ano in enumerate(range(ANO_MIN, ANO_MAX + 1), start=1):
        ...
        if out_path.exists():
            print(f"[{i:02d}/{n_anos}] {out_path.name} — ja existe, pulando")
            continue
        ...
        if raw_path.exists():
            ...
            if not gee_inicializado:
                ...
                geom, gdf_mg = carregar_geometria_mg()
        ...
        compor_mapa(raw, ano, gdf_mg_local, out_path)
        print(f"[{i:02d}/{n_anos}] {out_path.name}  ({time.time() - t0:.1f}s, {origem})")

    print(f"OK — {n_anos} mapas em {OUT_DIR}")
